...
export function registerOutboundRoutes(fastify) {
  ...
  if (
    ...
    throw new Error(
      "Missing required environment variables for outbound calls"
    );
  }
  ...
  fastify.post("/outgoing-call", async (request, reply) => {
    ...
    try {
      const call = await twilioClient.calls.create({
        ...
        }/outgoing-call-twiml?firstMessage=${encodeURIComponent(
          firstMessage
      ...
      (connection, req) => {
        ...
        elevenLabsWs.on("message", (data) => {
          try {
            ...
            console.error(
              "[ERROR] ElevenLabs message processing failed:",
              error.message
            );
          }
        });
